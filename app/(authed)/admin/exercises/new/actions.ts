...
export async function createExerciseAction(_prev: ExerciseFormState, formData: FormData): Promise<ExerciseFormState> {
  ...
  const { error } = await adminClient.from('exercises').insert({
    title, description: description || null, video_url: video_url || null, muscle_groups, tags: tagArray,
  } as never);

  if (error) return { status: 'error', error: 'Failed to create exercise. Please try again.' };
